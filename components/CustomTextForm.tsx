...
export default function CustomTextForm({ onSubmit }: CustomTextFormProps) {
    ...
    const handleParseAndSubmit = (e: React.FormEvent) => {
        ...
        // Parse custom text format: "Dutch sentence = Turkish sentence" per line.
        // Or if they just paste Dutch text, maybe we parse by periods? 
        // The user explicitly stated: "ilk cumle Hollandaca = isaretinden sonra da Turkce anlami."
            ...
            if (line.includes('=')) {
                ...
                if (parts.length >= 2) {
                    ...
                    parsedLines.push({
                        ...
                        targetText: dutch,
                        nativeText: turkish,
                        pauseMultiplier: 1.5 // default safe multiplier
                    });
                }
            } else {
                // If no "=" is found, just read the line as Dutch, and leave Turkish blank
                parsedLines.push({
                    id: i + 1,
                    targetText: line,
                    nativeText: '',
        ...
        const customScenario: Scenario = {
            title: title.trim(),
            targetLang: 'nl-NL',
            nativeLang: 'tr-TR', // Assuming Turkish native for the custom input based on user request
            lines: parsedLines,
        };
    ...
    return (
        <form onSubmit={handleParseAndSubmit} className="flex flex-col gap-6 w-full max-w-md mx-auto">
            ...
            {/* Textarea */}
            <div className="flex flex-col gap-2">
                <label className="text-gray-400 text-sm font-medium uppercase tracking-wider flex justify-between">
                    <span>Metniniz</span>
                    <span className="text-gray-500 text-xs normal-case">Format: Hollandaca = Türkçe</span>
                </label>
                <textarea
                    value={rawText}
                    ...
                    rows={8}
                    className="w-full bg-shadow-800 border border-gray-700 rounded-2xl px-5 py-4 text-white text-base
                     placeholder-gray-500 focus:outline-none focus:border-neon-green focus:ring-2
